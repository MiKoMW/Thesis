...
This idea was firstly developed by Ritter et al\cite{ritter2011data}. (cited in Jurafsky and Martin\cite{jurafsky2019speech}) using phrase-based machine translation to translate a user turn to a system response. In 2015, transduction models for response generation were modelled using encoder-decoder (seq2seq) models\cite{shang2015neural,strub2017end,sordoni2015neural}. However, the simple seq2seq generation architecture is unable to model the prior context of the conversation. Serban et al\cite{serban2016building} suggests a hierarchical model (HRED) that summarizes information over multiple prior turns. This model consists of two recurrent neural networks (RNNs) stacked on top of each other: one is a sentence-level RNN which encodes each utterance into a fixed length vector, while a conversation-level RNN takes each utterance vector as input and outputs a vector that summarizes the conversation so far. The vector is mapped back to text using a recurrent decoder. This gives a way for the previous information to be passed to future turns as hidden states\cite{lowe2017training}.
...
\multirow{9}{*}{\begin{tabular}[c]{@{}l@{}}Human-human \\ spontaneous\\ corpora\end{tabular}} & Switchboard\cite{godfrey1992switchboard}                                                                                        & 1992                      & Spoken        & 2,400                                                          & 3M                                                        &                                                                                                            \\ \cline{2-7} 
                                                                                              ...
                                                                                              & Topical Chat\cite{Gopalakrishnan2019}                                                                                           & \multicolumn{1}{l|}{2019} & Online Chat   & 10784                                                          & 4M                                                        & \begin{tabular}[c]{@{}l@{}}Links between plain text \\ knowledge and conversation.\end{tabular} \\ \hline
\end{tabular}
...
\multirow{6}{*}{\begin{tabular}[c]{@{}l@{}}Human-human\\ Goal-oriented\\ Corpora\end{tabular}} & HCRC Map Task Corpus\cite{anderson1991hcrc}                       & 1991                      & Spoken              & 128                                                            & 147K                                                      & \begin{tabular}[c]{@{}l@{}}Dialogues from HLAP Task in which \\ speakers must collaborate verbally\\ to reproduce on one participants \\ map a route printed on the others.\end{tabular} \\ \cline{2-7} 
                                                                                               ...
                                                                                               & GuessWhat\cite{de2017guesswhat}                                                         & 2017                      & Mini Game           & 160K                                                           & 4M                                                        & \begin{tabular}[c]{@{}l@{}}Identify an object in a complex visual\\ scene by asking a sequence of \\ yes/no questions.\end{tabular}                                                       \\ \cline{2-7} 
                                                                                               & Complex Sequential Question Answering\cite{saha2018complex}           & 2010                      & Semi-auto Generated & 169K                                                           &                                                           & \begin{tabular}[c]{@{}l@{}}Sequential Question Answering Utterance\\ generated based on human defined template.\end{tabular}                                                               \\ \cline{2-7} 
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=1\textwidth]{qa1.png}
%     \caption{Question Answering Interface for Questioner(above) and Answerer(below).}
%     \label{fig:kgjson}
% \end{figure}
...
\begin{enumerate}
   ...
   \item \textbf{Knowledge graphs are unambiguous.} Different people have different way to interpret a sentence and label a text span. This makes the evaluation of the text-based question-answering task very challenging. As we mentioned above, humans usually under-perform in such tasks. Unlike plain text, knowledge graphs are unambiguous. Accurate evaluation is possible under this framework.
   ...
   \item  \textbf{Knowledge graphs are structural.} Knowledge graphs are structural in the sense that the relations between entities are easily interpretable. This makes it extremely easy to control the topics and topic switches of the conversations.
    ...
    \item \textbf{Knowledge graphs are well-studied.} There are many prior works related to knowledge graphs, such as embedding\cite{lin2015learning}. In addition, a knowledge graph is a directed graph, there are many algorithms (e.g. depth first search) we could use for our study.
...
For each conversation, there will be 6 relations and answer nodes highlighted in sequence. The sequence of the highlighted relations together with the highlighted nodes, which we call a path, is predefined in order to control the topic shifts of each conversation. Different patterns of topic shifts (paths) will have influences on the appearances of various linguistic patterns. In Table \ref{tab:terms}, we would like to introduce some terminologies before we investigate those patterns. Figure \ref{fig:gaps} and Figure \ref{fig:links} are example sequences of gaps and links.
...
\caption{Definition of our Tag Set.}
\label{tab:tags}
\end{table}
...
This section introduces the tool-kits have been developed and used during this project. In addition, we briefly discuss the technical difficulties and their solutions during the project.
...
Emailing students is the most effective approach by which we recruit most of the participants. An alternative approach is to deploy this experiment to crowd-sourcing platform, such as Amazon Mechanic Turk\cite{mturk}. Hundreds of participants could be recruited overnight if the experiment is reasonably paid.
...
Our corpus contains natural extended conversations grounded on an external wide-coverage structural knowledge graph. At present, there is no such dataset existing (see Table \ref{tab:compare}) and this is what our corpus mainly developed for.
...
In previous chapter, we introduce a novel conversational question answering corpus. Our corpus contains 240 natural conversations, each has 6 question-answering turns, on 10 distinct topics. These conversations are divided into two categories: continuous and discontinuous conversations. There are 140 continuous and 100 discontinuous conversations. Discontinuous conversations are additionally split into two subgroups, discontinuous1 (conversations with only one gap) and discontinuous2 (conversations with many gaps). There are 43 ``dis1" conversations and 57 ``dis2" conversations. Table \ref{tab:topiccount} shows a detailed count for conversations of each topic in each category.
...
According to Table \ref{tab:agree}, Anaphora and Short Answer have a almost perfect agreement. Labels such as ``Additional information outside graph" and ``Conversation Control" have a lower Kappa score. This behaviour is expected and there are several reasons:

\begin{enumerate}

    \item According to the annotation schema (Appendix \ref{appendix:annotation}), the criteria for tags, such as Anaphora, is less ambiguous. Different interpretation of the semantics of a sentence usually doesn't influence on the judgment of that tag.
   ...
   \item Tags, like Anphora, can usually be determined by the grammars instead of semantics of a sentence. 
...
For example, Table \ref{tab:ori} shows that there are 2880 utterances in the dataset. 1197 of them contain Anaphora tag which consists 41.56\% of these utterances.
...
Total Number of Utterance           & \multicolumn{2}{l|}{1680}        \\ \hline
Tags                                 & Count & \# per Utterance \\ \hline
...
Total Number of Utterance           & \multicolumn{2}{l|}{1200}          \\ \hline
Tags                                 & Count  & \# per Utterance  \\ \hline
...
Total Number of Utterance           & \multicolumn{2}{l|}{516}            \\ \hline
Tags                                 & Count   & \# per Utterance  \\ \hline
...
Total Number of Utterance           & \multicolumn{2}{l|}{684}            \\ \hline
Tags                                 & Count   & \# per Utterance  \\ \hline
Anaphora                             & 218     & 0.3187                    \\ \hline
...
Total Number of Utterance           & \multicolumn{2}{l|}{840}                  \\ \hline
Tags                                 & Count      & \# per Utterance     \\ \hline
...
Total Number of Utterance           & \multicolumn{2}{l|}{600}                    \\ \hline
Tags                                 & Count       & \# per Utterance      \\ \hline
...
Total Number of Utterance           & \multicolumn{2}{l|}{993}                  \\ \hline
Tags                                 & Count      & \# per Utterance     \\ \hline
...
Total Number of Utterance           & \multicolumn{2}{l|}{447}               \\ \hline
Tags                                 & Count    & \# per Utterance    \\ \hline
...
Total Number of Utterance           & \multicolumn{2}{l|}{98}                          \\ \hline
Tags                                 & Count         & \# per Utterance         \\ \hline
...
Total Number of Utterance           & \multicolumn{2}{l|}{895}                           \\ \hline
Tags                                 & Count          & \# per Utterance          \\ \hline
...
\begin{observation}
Links have an obvious influence on Sluicing. The utterance with a link to its previous utterance is likely to contain a Sluicing construction. Links don't have an obvious influence on Anaphor.
\end{observation}

Based on this observation, we make two hypothesises:

\begin{hypo}
Links have an influence on the appearance of Sluicing. The utterance with a link to its previous utterance is likely to contain a Sluicing construction.
\end{hypo}
...
All the examples and patterns we mentioned above is extremely challenging and unachievable for the current state of the art systems. Building coherent dialogue systems involves many fundamental problems of artificial intelligence and will remain a central topic for the research communities.
...
We prove the statistical significance of those hypothesis on our corpus. We also show how challenging and exciting to model coherent dialogues via examples in our corpus. Finally, building coherent dialogue systems will remain our research interests for the future.
